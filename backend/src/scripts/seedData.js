...
const devices = [
  {
    ...
    model: "ASC-PRO-2026",
    description:
      "Advanced irrigation controller with AI-powered water management, solar panel, and real-time monitoring.",
    features: [
      "AI-powered water optimization",
  ...
  },
  {
    ...
    model: "ASK-ADV-2026",
    description:
      "Complete sensor kit for precision agriculture with soil moisture, temperature, and humidity sensors.",
    features: [
  ...
  },
  {
    ...
    model: "SWP-500-2026",
    description:
      "Efficient solar-powered water pump system for irrigation with automatic control.",
    features: [
      "500W solar pump",
  ...
  },
  {
    ...
    model: "AFG-MINI-2026",
    description:
      "Compact field gateway for connecting edge sensors to cloud dashboard.",
    features: [
      "Dual SIM fallback",
      "Low-power mode",
  ...
  },
  {
    ...
    model: "PDC-X2-2026",
    description:
      "Two-channel precision drip controller for vineyards and orchards.",
    features: [
...
const discussions = [
  {
    ...
    content:
      "I have a 10-hectare cotton field and want to implement drip irrigation. What spacing should I use for the drip lines?",
  ...
  },
  {
    ...
    content:
      "My tomato plants are showing leaf curl symptoms. Leaves are yellowing and curling upward.",
  ...
  },
  {
    ...
    content:
      "My soil test shows pH 5.8, but grapes prefer 6.0-6.5. Which amendments should I apply?",
  ...
  },
  {
    ...
    content:
      "In dusty region, panels lose efficiency quickly. How often should I clean and what method is safest?",
  ...
  },
  {
    ...
    content:
      "Tank level drops by 20% overnight even when irrigation is off. Could this be a leak or sensor issue?",
    problemType: "other",
...
async function seedDatabase() {
  try {
    ...
    const createdDiscussions = await Discussion.insertMany(
      discussionsWithAuthors,
    );

    createdDiscussions[0].replies.push({
      author: createdUsers[0]._id,
      content:
        "For cotton fields, use 60-75cm spacing between drip lines and keep pressure around 1.0-1.5 bar.",
      isHelpful: true,
    ...
    createdDiscussions[0].replies.push({
      ...
      content:
        "We also reduced clogging by adding a 120-micron filter before each zone valve.",
      isHelpful: false,
    });
    ...
    createdDiscussions[3].replies.push({
      author: createdUsers[0]._id,
      content:
        "Use a soft brush and clean early morning; avoid abrasive cleaning during peak sunlight.",
      isHelpful: true,
    ...
    const farmerGroups = [
      {
        ...
        description:
          "Community for cotton farmers to share experiences and best practices",
        ...
        posts: [
          {
            ...
            content:
              "Welcome to our cotton farming community! Feel free to share your experiences and ask questions.",
      ...
      },
      {
        ...
        description:
          "Group for users of smart irrigation systems and technology",
        ...
        posts: [
          {
            author: createdUsers[0]._id,
            content: "Share your smart irrigation setups and tips here!",
            createdAt: new Date("2026-01-20"),
          },
          {
            author: createdUsers[2]._id,
            content: "Just installed AGRANOVA Controller Pro - works amazing!",
            createdAt: new Date("2026-02-01"),
          },
        ],
      },
      {
        ...
        description:
          "For farmers interested in organic and sustainable agriculture",
      ...
      },
      {
        ...
        description:
          "Discuss agricultural equipment, maintenance, and recommendations",
    ...
    for (const group of createdFarmerGroups) {
      const membersToAdd = createdUsers
        .slice(0, 5)
        .filter(
          (user) =>
            !group.members.some((member) => member.user.equals(user._id)),
        );
    ...
    const messages = [
      {
        group: chatGroups[0]._id,
        sender: createdUsers[1]._id,
        content:
          "Pump starts and stops every 5 minutes. Could this be pressure instability?",
      ...
      },
      {
        ...
        content:
          "Check line pressure and clean the inlet filter. Cycling often means pressure drop.",
        timestamp: new Date(Date.now() - 110 * 60 * 1000),
      },
      {
        group: chatGroups[0]._id,
        sender: createdUsers[2]._id,
        content: "We fixed this by replacing a leaking check valve in zone 3.",
        timestamp: new Date(Date.now() - 100 * 60 * 1000),
      },
      {
        group: chatGroups[1]._id,
        sender: createdUsers[3]._id,
        content: "Is gypsum enough to reduce sodium in clay soil?",
      ...
      },
      {
        ...
        content:
          "Use gypsum with proper drainage; do a follow-up soil test in 4-6 weeks.",
        timestamp: new Date(Date.now() - 80 * 60 * 1000),
      },
      {
        group: chatGroups[2]._id,
        sender: createdUsers[7]._id,
        content:
          "Strong winds expected tomorrow in Gabala. Secure the sensor masts.",
    ...
    const questions = await Question.insertMany([
      {
        ...
        content:
          "Soil moisture ranges between 38-52%. How many cycles per day are optimal in hot weeks?",
        ...
        answers: [
          {
            ...
            content:
              "Use shorter, more frequent cycles and keep root-zone moisture near field capacity.",
            isBestAnswer: true,
            votes: 7,
          },
          {
            user: createdUsers[5]._id,
            content:
              "We run 2 morning cycles + 1 evening cycle during heat peaks.",
            votes: 3,
          },
        ],
      },
      {
        ...
        content:
          "New sensors show high variance between nearby probes. What is a reliable calibration process?",
        ...
        answers: [
          {
            user: createdUsers[0]._id,
            content:
              "Calibrate against gravimetric samples at two moisture points: near dry and field capacity.",
            votes: 5,
          },
        ],
      },
      {
        ...
        content:
          "I want to reduce evaporation losses. Is full night irrigation recommended?",
    ...
    console.log(
      `✅ Created ${chatGroups.length} groups, ${createdMessages.length} messages, ${questions.length} questions`,
    );
    ...
    for (let hour = 47; hour >= 0; hour -= 1) {
      ...
      sensorHistory.push({
        soilMoisture: Math.max(
          18,
          Math.min(78, 48 + Math.floor(Math.sin(hour / 4) * 14)),
        ),
        temperature: Math.max(
          13,
          Math.min(36, 24 + Math.floor(Math.cos(hour / 6) * 6)),
        ),
        humidity: Math.max(
          30,
          Math.min(88, 58 + Math.floor(Math.sin(hour / 5) * 15)),
        ),
        waterTankLevel: Math.max(12, Math.min(92, 72 - (47 - hour) * 1.1)),
        pumpStatus: hour % 5 === 0 ? "ON" : "OFF",
        solarPanelStatus: randomChoice(
          ["ACTIVE", "CHARGING", "ACTIVE", "INACTIVE"],
          hour,
        ),
    ...
    createdSystems.forEach((system, systemIndex) => {
      for (let i = 0; i < 12; i += 1) {
        logRows.push({
          ...
          logType: randomChoice(
            ["INFO", "WARNING", "SYSTEM", "SENSOR", "NETWORK"],
            i + systemIndex,
          ),
          action: randomChoice(
            [
          ...
          data: {
            soilMoisture: Math.max(
              10,
              Math.min(85, system.soilMoisture + (i % 3) - 1),
            ),
