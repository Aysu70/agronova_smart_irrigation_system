...
import { DEMO_SENSOR } from "../utils/demoData";
...
const Dashboard = () => {
  ...
  const fetchLatestData = async () => {
    try {
      ...
      if (data) {
        ...
        // fallback to demo sensor when response shape unexpected
        setSensorData(DEMO_SENSOR);
        toast("Using demo sensor data", { icon: "🧪" });
      }
    } catch (error) {
      // Use demo sensor data so dashboard remains functional for testing
      setSensorData(DEMO_SENSOR);
      toast("Failed to load live sensor data — showing demo values", {
        icon: "🧪",
      });
    }
    setLoading(false);
  ...
  const connectBluetooth = async () => {
    if (!bluetoothService.isSupported()) {
      toast.error(
        "Web Bluetooth is not supported. Use Chrome, Edge, or Opera.",
      );
      return;
    }
    ...
    // Show scanning toast
    const scanningToast = toast.loading(
      ...
      <div className="ml-64">
        ...
        <div className="p-8 mt-16">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              {!deviceConnected ? (
                <button
                  ...
                  disabled={
                    isConnecting || isScanning || connectionType === "wifi"
                  }
                ...
                <button
                  onClick={
                    bluetoothConnected ? disconnectBluetooth : handleDisconnect
                  }
            ...
            {connectionType === "wifi" && !deviceConnected && (
              <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-sm text-blue-800">
                  <strong>WiFi Mode:</strong> ESP32 devices connect
                  automatically. Make sure your ESP32 is powered on and
                  configured with your server IP address.
                </p>
              </div>
            )}
              ...
              !bluetoothService.isSupported() && (
                <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                  <p className="text-sm text-yellow-800">
                    <strong>Browser Not Supported:</strong> Web Bluetooth only
          ...
          >
            ...
            <p
              className={
                deviceConnected ? "text-green-700" : "text-primary-700"
              }
            >
              {deviceConnected
