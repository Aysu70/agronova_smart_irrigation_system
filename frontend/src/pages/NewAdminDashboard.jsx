...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const users =
        usersResponse?.data?.data && usersResponse.data.data.length > 0
          ? usersResponse.data.data
          : DEMO_USERS;

      // Calculate stats
      const paymentsTotal =
        DEMO_PAYMENTS && DEMO_PAYMENTS.length > 0
          ? DEMO_PAYMENTS.reduce((s, p) => s + (p.amount || 0), 0)
          : users.length * 150;
      setStats({
        totalUsers: users.length,
        activeDevices:
          (DEMO_DEVICES && DEMO_DEVICES.length) ||
          users.filter((u) => u.role === "user").length, // Simulated
        orderedDevices: users.length * 2, // Simulated
        totalPayments: paymentsTotal, // Simulated or demo
      });
      ...
      // If there's no real orders, use demo orders
      setRecentOrders(DEMO_ORDERS.slice(0, 3));

      // Prefer demo payments list when available
      setRecentPayments(
        (DEMO_PAYMENTS && DEMO_PAYMENTS.length > 0
          ? DEMO_PAYMENTS
          : DEMO_ORDERS
      ...
      // Fallback to demo data so admin UI remains usable
      setStats({
        totalUsers: DEMO_USERS.length,
        activeDevices:
          (DEMO_DEVICES && DEMO_DEVICES.length) ||
          DEMO_USERS.filter((u) => u.role === "user").length,
        orderedDevices: DEMO_USERS.length * 2,
        totalPayments:
          DEMO_PAYMENTS && DEMO_PAYMENTS.length > 0
            ? DEMO_PAYMENTS.reduce((s, p) => s + (p.amount || 0), 0)
            : DEMO_USERS.length * 150,
      });
      setRecentUsers(DEMO_USERS.slice(0, 5));
      setRecentOrders(DEMO_ORDERS.slice(0, 3));
      setRecentPayments(
        (DEMO_PAYMENTS && DEMO_PAYMENTS.length > 0
          ? DEMO_PAYMENTS
      ...
      <div className="p-6 bg-gray-50 min-h-screen">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Manage users, orders, payments, and system operations
          </p>
        </div>
