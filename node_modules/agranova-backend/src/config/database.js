...
const connectDB = async () => {
  try {
    ...
    const host =
      conn &&
      conn.connection &&
      (conn.connection.host || conn.connection.hosts || "localhost");
    const port =
      conn &&
      conn.connection &&
      (conn.connection.port ||
        (conn.connection.hosts &&
          conn.connection.hosts[0] &&
          conn.connection.hosts[0].port) ||
        "27017");
    // Safely read database name in case mongoose/mongo driver versions differ
    const dbName =
      (conn &&
        conn.connection &&
        conn.connection.db &&
        (conn.connection.db.databaseName ||
          (conn.connection.db.s && conn.connection.db.s.databaseName))) ||
